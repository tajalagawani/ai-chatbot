...
useEffect(() => {
  if (!metadata?.outputs || metadata.outputs.length === 0) {
    ...
    const errorCount = (metadata?.outputs || [])
      .filter(output => output.status === 'failed').length;
    
    const successCount = (metadata?.outputs || [])
      .filter(output => output.status === 'completed').length;

    // Function to handle the console's minimize button click
    const handleConsoleMinimize = useCallback(() => {
      // Instead of letting the Console component handle this internally,
      // we'll set consoleExpanded to false in our metadata
      setMetadata(prev => ({
        ...prev,
        consoleExpanded: false
      }));
    ...
    return (
      <div className="relative w-full h-full">
        ...
        {/* Console in Footer - Just the Header Bar when collapsed */}
        {!metadata?.isFlowFullscreen && (
          <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-[#1a1a1a]">
            {metadata?.consoleExpanded ? (
              <div>
                {/* Insert a custom header that will override the Console's built-in header */}
                <div className="flex flex-row justify-between items-center w-full h-10 px-2 py-1 bg-[#0f0f10] border-b border-[#1a1a1a]">
                  <div className="text-sm pl-2 text-[#e2e8f0] flex flex-row gap-3 items-center">
                    ...
                    <div className="text-xs text-[#4b5563]">
                      {metadata?.outputs?.length || 0} output{(metadata?.outputs?.length || 0) !== 1 ? 's' : ''}
                    </div>
                  </div>
                ...
                {/* Render the Console with a modified height to account for our custom header */}
