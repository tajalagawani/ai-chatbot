...
const JsonOut = ({ code, editable = false, onChange = () => {} }) => {
  ...
  // Highlight error values in the JSON output
  const highlightedLines = useMemo(() => {
    return codeLines.map((line) => {
      if (line.includes('"error":')) {
        ...
        return (
          <>
            {parts[0]}"error": <span className="text-red-400">{parts[1]}</span>
          </>
        )
      }
      return line || " "
...
const SchemaView = React.memo(({ schema, data, onDragStart, sourceNodeId }: SchemaViewProps) => {
  ...
  const renderTree = useCallback(
    (key: string | undefined, value: any, dataValue: any, path = "") => {
      ...
      // Only apply red text to error field values
      const isErrorValue = key === "error"
      ...
      const titleContent = (
        ...
        >
          ...
          {value.type !== "object" && value.type !== "array" && (
            <span className={`text-sm truncate max-w-[50%] ${isErrorValue ? "text-red-400" : "text-gray-500"}`}>
          ...
          <div key={fullPath} className="mb-2">
            <Accordion type="single" collapsible defaultValue={fullPath}>
              <AccordionItem value={fullPath}>
                <AccordionTrigger className="px-2 hover:no-underline">{titleContent}</AccordionTrigger>
                <AccordionContent>
                  <div className="ml-4">
                    {Object.entries(value.properties).map(([k, v]) => renderTree(k, v, dataValue?.[k], fullPath))}
                  </div>
                </AccordionContent>
              </AccordionItem>
            </Accordion>
                  ...
                  <div className="ml-4">
                    {Array.isArray(dataValue) && dataValue.length > 0 ? (
                      <div>
                        ...
                        {dataValue.length > 3 && (
                          <div className="text-xs text-gray-500 italic">...and {dataValue.length - 3} more items</div>
                        )}
                      </div>
                    ) : (
          ...
          <div key={fullPath} className="mb-2">
            <Accordion type="single" collapsible defaultValue={fullPath}>
              <AccordionItem value={fullPath}>
                ...
                <AccordionContent>
                  <pre
                    className={`text-xs mt-2 whitespace-pre-wrap break-words p-2 rounded bg-[#0f0f10] ${isErrorValue ? "text-red-400" : "text-gray-300"}`}
                  >
  ...
  return (
    <div className="font-mono text-sm overflow-y-auto h-full">
      {Object.entries(schema.properties).map(([key, value]) => renderTree(key, value, data[key]))}
    </div>
  )
})
...
const DataViewer: React.FC<DataViewerProps> = ({
      ...
      try {
        if (Object.keys(value).length > 3) {
          return JSON.stringify({
            ...Object.fromEntries(Object.entries(value).slice(0, 3)),
            "...": "...",
          })
        }
  ...
  const renderCell = useCallback(
    (item: DraggableItem, columnKey: React.Key) => {
      const cellValue = item[columnKey as keyof DraggableItem]
      // Only apply red text to error field values
      const isErrorValue = item.name === "error"
        ...
        case "value":
          return (
            <div className="max-w-[300px] overflow-hidden">
              <p className={`text-sm break-all truncate ${isErrorValue ? "text-red-400" : ""}`}>
                {formatValue(cellValue, item.name)}
              </p>
            </div>
          )
        case "type":
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex-1 overflow-auto">
        {activeTab === "schema" && (
          <SchemaView schema={schema} data={data} onDragStart={onDragStart} sourceNodeId={nodeId} />
        )}
        {activeTab === "json" && <JsonOut code={JSON.stringify(data, null, 2)} editable={false} />}
        {activeTab === "table" && (
              ...
              <div className="flex w-full justify-center py-4 border-t">
                <Pagination>
                  <PaginationContent>
                    ...
                    <PaginationNext onClick={() => setPage((p) => Math.min(pages, p + 1))} disabled={page === pages} />
                  </PaginationContent>
                </Pagination>
              </div>
            )}
...
export const getStatusIcon = (status) => {
  const CheckCircle = (props) => (
    <svg
      ...
      width="24"
      height="24"
      className={props.className || "h-4 w-4 text-green-500"}
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
  ...
  const Clock = (props) => (
    <svg
      ...
      width="24"
      height="24"
      className={props.className || "h-4 w-4 text-blue-500"}
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
